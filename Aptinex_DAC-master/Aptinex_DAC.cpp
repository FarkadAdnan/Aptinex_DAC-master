 ...
 */
Aptinex_4CH_DAC::Aptinex_4CH_DAC(void) {}
 ...
 */
boolean Aptinex_4CH_DAC::begin(uint8_t i2c_address, TwoWire *wire) {
  _addr = i2c_address;
  _wire = wire;
  _wire->begin();
 ...
 * and the DAC
 */
bool Aptinex_4CH_DAC::setChannelValue(
  ...
  if (_wire->write(output_buffer, 3) != 3) {
    return false;
  }
 ...
 * and the DAC
 */
bool Aptinex_4CH_DAC::fastWrite(uint16_t channel_a_value,
  ...
  if (_wire->write(output_buffer, 8) != 8) {
    return false;
  }
  if (_wire->endTransmission(true) == 0) {
    return true;
...
bool Aptinex_4CH_DAC::saveToEEPROM(void) {
  uint8_t input_buffer[24];
  uint8_t output_buffer[9];

  //i2c_dev->read(input_buffer, 24);
  size_t recv = _wire->requestFrom((uint8_t)_addr,(uint8_t) 24, (uint8_t)true);
  if (recv != 24)return false;
  for (uint16_t i = 0; i < 24; i++)input_buffer[i] = _wire->read();
  return true;
  ...
  if (_wire->write(output_buffer, 9) != 9) {
    return false;
  }
  if (_wire->endTransmission(true) == 0) {
    return true;
